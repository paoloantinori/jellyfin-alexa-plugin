...
2. `HandleAsync`:
   - Resolve the `playlist` slot value → playlist item + members, **reusing `PlayPlaylistIntentHandler`'s resolution logic** (`PlaylistTrackResolver`) — extract to a shared helper if not already cleanly callable; preferred over duplicating the Jellyfin playlist-members call. Reuses the existing locale response keys (`NotFoundPlaylist`, `PlaylistEmpty`, `DidNotCatchPlaylistName`) — **no new response strings**.
