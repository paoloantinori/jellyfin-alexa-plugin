...
Documented in the Standard Built-in Intents table (verified 2026-09-07, sharpened by the 2026-09-14 deep-research pass with per-claim adversarial votes): while the skill **is playing audio or was the most recent skill to play audio**, users can invoke these intents **WITHOUT the invocation name**: exactly ELEVEN (Cancel, LoopOff, LoopOn, Next, Pause, Previous, Repeat, Resume, ShuffleOff, ShuffleOn, StartOver); `AMAZON.PauseIntent` and `AMAZON.ResumeIntent` are MUST-implement, the other nine are delivered **even if not in the intent schema**. `AMAZON.StopIntent` is NOT in the set (0 of 10 locale tables attach the without-name clause to it): it is the exit-skill intent; its response must have `shouldEndSession` `true` or `null`, and implementing it is a certification requirement. The bare "Alexa, stop" heard during our playback is documented to be delivered to the most-recently-playing skill as `AMAZON.PauseIntent` (verified 6-0). CAUTION: the documented memory-reset rule (another skill plays, another streaming service is invoked, device reboot) must NOT be cited as the CAUSE of the default-music-service misrouting; the adversarial round refuted that causal link (1-2) and no official page documents any arbitration rule for when the default music service wins during our own playback. See claudedocs/research_stop-intent-routing_2026-09-14.md.
...
- **AudioPlayer playback** (verified on-device 2026-07-02, pause re-confirmed 2026-09-07): Amazon auto-routes ONLY **Pause/Resume** to the active skill. **Stop/Next/Previous are frequently claimed by the device's default music service** (Amazon Music/Spotify): zero `StopIntent`/`NextIntent`/`PlaybackStopped` events for "stop"/"ferma"/"avanti"; Alexa simulator `ConsideredIntents` = `<IntentForDifferentSkill>`. A fresh content request ("play a different playlist/album/artist/song") is likewise NOT auto-routed; it goes to the default music service. Not fixable plugin-side: custom `AudioPlayer` skills cannot claim the device's default-music slot (reserved for Music Skill API skills; corrected 2026-09-19 per the JF-561 research: MSAPI is self-serve for US distribution, NOT partnership-only, but its interfaces support ONLY en-US/es-US, so the slot remains unreachable for this it-IT household and for any non-US user of the plugin; same reason custom skills get no seek bar). `PlaybackController` is NOT a fix (per Amazon docs it serves hardware buttons only, has no STOP op, and is never sent for voice). Keeping the session open does NOT help (and `JF-299` shows `shouldEndSession=false` is harmful). **Third confirmation 2026-09-14, hole closed**: "stop" spoken to the PLAYING device itself (Echo Show, the same one rendering our NowPlaying APL; the screen being ours does NOT confer voice routing), ASR-transcribed CLEAN ("stop" verbatim in the Alexa app voice history; that screen shows no handled-by attribution), still ZERO requests reached the endpoint; our StopIntent handler verified via simulator to answer `AudioPlayer.Stop` + `ShouldEndSession=true` when a stop does arrive. Bare "stop" is also device-LOCAL: it cannot stop skill audio playing on a DIFFERENT Echo.
...
- `AMAZON.StopIntent`/`AMAZON.CancelIntent`: `AudioPlayerStop()` + `ShouldEndSession=true`. `AMAZON.PauseIntent`: same, plus optional position card, UNLESS `PluginConfiguration.PauseKeepsSession` is on (JF-482 experiment, DEFAULT TRUE): then the response keeps the session open with a "riprendi" reprompt (audio still stops via the directive; the device opens the mic ~8s, `ExpectSpeech`). All paths send the `AudioPlayer.Stop` directive to guarantee audio stops. Never `ResponseBuilder.Empty()` for stop/cancel; it lacks the directive.
...
When a handler's primary search finds no results (e.g., PlaySongIntent finds no song), `BuildArtistSongsResponseAsync` falls back to artist search. This is shared across PlaySong, PlayAlbum, and PlayVideo handlers via the `CrossMedia` collaborator (`CrossMediaFallback`, moved off BaseHandler in JF-315 batch 7; handlers reach it as `CrossMedia.X`).

`TryEntityFallbackAsync` extends this to **greedy free-text slots that misroute an artist query** (PlayMoodMusic's `mood` slot captures "di miles davis"; FindSong's `titleKeywords` when no artist was given; PlayByGenre's `genre` slot captures a bare title in the es locales, JF-463). On a confirmed miss it strips locale stop-words (`KeywordMatcher.Tokenize`, which covers all 11 language prefixes of the 17 locales since JF-389), runs the phonetic `ArtistSearch`, gates on `Math.Max(FuzzyMatcher.GetDefaultThreshold(user), CrossMediaArtistThreshold)` **and** the word-count guard `CrossMediaArtistMaxWords` (=2, on the CrossMediaFallback collaborator; a >2-word slot is a poor artist query), then plays via `BuildArtistSongsResponseAsync` with a `FoundArtistInstead` announcement. Returns null when no confident match so the caller falls through to its own not-found. Known tradeoff: a real single-word mood that coincidentally matches an artist name (>=85) substitutes it (announced, so the user knows). Do NOT remove the word-count guard (a PlaySong lesson: long queries match wrong short artists).
...
- **Jellyfin has no podcast type (JF-373/JF-599)**: Jellyfin 10.11.x has NO `Podcast`/`AudioPodcast` item type. A `Series` is ALWAYS `MediaType=Unknown` (a container rollup, never `Audio`), so any `IncludeItemTypes=Series` + `MediaTypes=Audio` query returns zero results unconditionally. Podcasts are stored under TWO shapes: a **MusicAlbum of Audio tracks** in a Music library (all 3 community podcast plugins; episodes are direct children, `ParentId=album.Id`) or a **Series of Episode items under season folders** (the IlPost plugin: live-verified 2026-09-20, 78 series / 677 Episodes / ZERO MusicAlbums in that library; episodes sit under year folders so the series is an ANCESTOR, `AncestorIds=series.Id`; the Episode items are MediaType=Video by default but their media is MP3, and `/Audio/{id}/stream?static=true` serves them fine). `PlayPodcastIntentHandler` queries `MusicAlbum` first, falls back to `Series` on a miss (album shape stays primary and byte-identical), then plays the newest child (`OrderBy DateCreated Desc`). Do NOT re-introduce the Series+MediaTypes=Audio query (the MediaTypes filter is what made it dead, not the Series type).
- **AnnounceAudioPlays (JF-353 ext)**: The now-playing announce on MUSIC plays is a separate opt-in flag (`PluginConfiguration.AnnounceAudioPlays`, default `false`) — NOT the same as `DefaultAnnounceNowPlaying` (which gates video/book launches only, default `true`). `AttachAnnounceIfEnabled` in the `Launch` / PlaybackLaunchBuilder collaborator (JF-315 batch 4) reads the audio flag via `GetAnnounceAudioPlays(user)` (per-user override → global default). Both builders (`BuildAudioPlayerResponse` + `BuildVideoAppAudioResponse`) call it.
...
- **The Echo Show native now-playing surface COVERS any skill APL document during AudioPlayer playback** (JF-624, proven 2026-09-24 by a live card-drop experiment verified on the wire + corroborated by SO 50622919): the device synthesizes its full-screen player (`CardRenderer.PlayerInfo`, ~1s after `PlaybackStarted`) from the `AudioItem.Metadata` alone; a skill `RenderDocument` in the same response only flashes before playback starts and stays covered for the whole track. Dropping the StandardCard from APL-carrying responses does NOT change this (the metadata alone drives it). Consequences: (1) a live-updating NowPlaying screen during music playback is NOT attainable (our APL is covered; the native bar itself stays indeterminate for custom skills, scrubber is MSAPI-only); (2) JF-623's stale-screen fix worked by refreshing the metadata the native surface reads; (3) skill APL effort belongs on non-playback surfaces (lists, browse, disambiguation) and on the pre-playback window. The correctly-shaped `handleTick` + `SetValue` tick handler is kept in the enhanced template (proven working in the Amazon web simulator runtime) so any future window where the document is visible gets a live bar for free. APL tick-handler shape gotcha: the handler object REQUIRES a `commands` array (`{"minimumDelay":1000,"commands":[{...}]}`); putting command properties directly on the handler object is silently ignored, no error anywhere.
