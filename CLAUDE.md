...
- **Layer 1 (per-path entry guards)**: handlers whose request path would hit the cold DB call `IndexWarmingGate.EnsureReady` at entry (title-only FindSong/PlaySong on the SONG index; musician-scoped PlaySong, PlayArtistSongs inline copy, SearchMedia, PlayMoodMusic, PlayByGenre, AddToQueue, PlayNext, QueryArtistLibrary on the artist/song index their path uses), BEFORE the "searching" progressive response and AFTER the cancel-word escape hatch (an open Dialog.ElicitSlot flow must still cancel during warming). A second entry gate interleaves with it: the JF-467 music gate sits BEFORE the warming gate where the handler geometry allows (FindSong, PlayMoodMusic, PlayArtistSongs) and AFTER it in PlaySong/PlayAlbum, where the warming gate must stay before the slot elicitation; the per-handler order is documented in each gate's comment, keep it when editing.
