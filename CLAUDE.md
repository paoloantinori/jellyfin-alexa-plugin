...
- **The VideoApp interface has exactly ONE directive: `VideoApp.Launch`.** There is NO `VideoApp.Stop` and no pause directive: once video starts, the skill CANNOT programmatically dismiss or pause the player. `AMAZON.CancelIntent` is not supported with VideoApp; the back button is always shown and cannot be hidden.
