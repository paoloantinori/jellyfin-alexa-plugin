...
- **Layer 1 (per-path entry guards)**: handlers whose request path would hit the cold DB call `IndexWarmingGate.EnsureReady` at entry (title-only FindSong/PlaySong on the SONG index; musician-scoped PlaySong, PlayArtistSongs inline copy, SearchMedia, PlayMoodMusic, AddToQueue, PlayNext, QueryArtistLibrary on the artist/song index their path uses), BEFORE the "searching" progressive response and AFTER the cancel-word escape hatch (an open Dialog.ElicitSlot flow must still cancel during warming).
