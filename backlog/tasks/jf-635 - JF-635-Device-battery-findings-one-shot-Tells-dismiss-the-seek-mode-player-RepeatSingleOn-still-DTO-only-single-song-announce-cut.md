...
2026-09-26 08:50 THIRD battery - two platform truths settled on device: (A) THE KEEP-ALIVE IS REFUTED: the stars' confirmation carried the APL RenderDocument (verified on the wire, corr=12909817) and the stream STILL died. The 17:51 MediaInfo 'survival' was misread (he re-launched the album 17s later; the stream had died there too). CONSOLIDATED MODEL: during seek-mode VideoApp playback, ANY one-shot voice interaction (Tell, Tell+APL, Fallback) kills the stream; the stale previous VideoApp surface then shows (test 4: the Temple of the Dog UI reappeared after a Fallback). The keep-alive attach STAYS but reframed: it does not preserve playback, it replaces the stale surface with current info when the stream dies. (B) PROGRESSIVE SPEECH BEFORE VIDEOAPP.LAUNCH DOES NOT PLAY on this Echo Show: both the album announce (08:50:00, 'In riproduzione Temple of the Dog') and the single-song announce (08:50:41, 'In riproduzione Magnolia') were SENT progressive (logged, no failure), the device played NEITHER - matching battery 1's 'non sono sicuro abbia annunciato'. The seek mode is a SILENT-LAUNCH mode: final speech gets cut by the player, progressive gets skipped. The only announcement surface is the VideoItem metadata on screen. (C) JF-622's reminder feature WORKS END-TO-END for the first time (armed, confirmed 'tra 30 secondi', fired); the feature is the skill's voice-reminder (JF-272), unreachable since inception due to the swapped ctor args. (D) Sleep 'tra' forms: 'fermare la riproduzione tra trenta secondi' fell to Fallback; three 'tra {sleep_duration}' carriers added to it-IT, model rebuilt, profile-nlu verified PT30S/PT5M live.
<!-- SECTION:NOTES:END -->
