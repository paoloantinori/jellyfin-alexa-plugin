...
PHASE 3 (native-swap delta re-measurement) COMPLETE 2026-09-12 night run: the native it-IT swap went live at 00:36 via the JF-513 all-locales rebuild (the committed repo model IS the swapped one; the 02:08 catalog sync rebased on it), so the live state is native swap + catalog wiring - the intended production shape. Full it-IT suite on that state: 212 passed, 19 failed. Decomposition: 5 documented NLU leftovers (suona/pleia matrix; Suona musica dei queen/pink floyd; the stranger-episodio trim collateral - all five fail consistently on targeted re-run) + 1 TRANSIENT (Riproduci la stagione uno episodio tre di breaking bad: profile-nlu routes PlayEpisodeIntent with all three slots correctly filled; PASSES on targeted re-run) + 13 e2e-class failures (the chronic simulate-skill 'unexpected error' instability, documented in phase 2). VERDICT: the pre-phase-1 '~12 swap-attributable failures' are RETIRED - on the phase-1/2 fixed baseline the native swap costs ZERO attributable failures; repo and cloud now agree on provenance too. REMAINING in this task: (a) the musica-family decision (accept-and-repin 'Suona musica dei X' vs a phase-2 experiment; sample-level fixes exhausted per the phase-1 evidence), (b) the en stragglers (en-US 'Repeat the song', 'Play the song hotel california from the eagles', 'Play imagine by john lennon next'; en-CA 'Play a random movie'; en-IN 'play stranger things season two episode one'), (c) the 4 documented model-bug leftovers stay tracked here (matrix pair catalog-unfixable; stranger-episodio is the trim's deterministic collateral).

EN STRAGGLERS classified 2026-09-12 night run (probes on the live post-phase-3 state): (1) 'Repeat the song' FIXED - a genuine LoopSongOn sample gap (only the forever forms were trained); 'repeat the song' + 'repeat this song' added to all five en templates, deployed, and live-verified routing LoopSongOnIntent in en-US/GB/CA/IN (en-AU identical template). (2) 'Play imagine by john lennon next' NOT A BUG - intent correct (PlayNextIntent) and the knowledge-graph quote echo in the slot value is neutralized by KeywordMatcher.Tokenize (splits on non-alphanumerics, so quotes never reach the tokens); a StripEntityQuotes helper was implemented and REVERTED per the no-belt-and-suspenders rule (the tokenizer already provides the protection the helper would fake). (3) FILED 'Play the song hotel california from the eagles': the trained form 'play the song {song} from {musician}' EXISTS but the MusicRecording entity swallows the whole phrase into song with musician unfilled - entity-weighting class, needs its own probe campaign. (4) FILED en-CA 'Play a random movie': MediaType already carries movie as a video synonym, yet the NLU fills genre='movie' with media_type empty - slot-attribution quirk; candidate fixes are a handler-side genre->media_type remap for movie/film or a fixture re-pin. (5) FILED en-IN 'play stranger things season two episode one': PlayEpisodeIntent correct with season/episode filled but series_name EMPTY despite trained samples - SeriesName-catalog suspect (the live catalog replaced the static seed and Series was excluded from the phase-2 enrichment with a one-time disabled warning); ties into JF-493.
