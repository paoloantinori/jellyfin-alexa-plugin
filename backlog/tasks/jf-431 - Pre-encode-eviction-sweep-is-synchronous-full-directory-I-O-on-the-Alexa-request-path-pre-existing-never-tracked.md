...
- DECISION (Option A, measure-first): the sweep stays synchronous. Synthetic sparse cache trees, production API shape, median of 5 warm runs on the N100 deploy target: 2,250 files/2.0GB -> 6.3ms; 10,300/1.8GB -> 29.7ms warm / 34.4ms cold; realistic worst case (full 2048MB cap of 160KB segments, ~13,000 files) -> ~37ms warm / ~43ms cold, under the 50ms budget. Option B (cached-size ledger) rejected: stale-total risk for no measurable win; the JF-428 pin/floor semantics need the synchronous sweep. The full table lives in this task's notes (JF-448 review F8: the duplicate inline copy at the scan site was reduced to the constraint + this pointer).
