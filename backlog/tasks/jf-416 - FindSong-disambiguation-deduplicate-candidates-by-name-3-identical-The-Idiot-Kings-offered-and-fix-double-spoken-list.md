...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Deduplicate FindSong disambiguation candidates by song NAME (case-insensitive): when multiple items share the same name (same song from different albums/versions), keep only one representative in the top-4 list. Selection criteria for the representative: prefer the most-played (UserData.PlayCount), tiebreak by first occurrence in the scored list.
- [x] #2 The deduplication must preserve the original scored ORDER (highest score first) among the deduplicated survivors.
- [x] #3 If deduplication reduces the list below 4, fill remaining slots from the next-highest-scoring non-duplicate items (up to the original Take(4) window, i.e. scan up to the first 8 scored items to find up to 4 unique names).
- [x] #4 Fix the double-speaking: FindSongFoundMultiple already receives candidateNames as a format arg, but fullPrompt appends candidateNames AGAIN. Remove the duplicate append.
- [x] #5 Unit tests: (a) 3 items with same name + 1 different -> disambiguation shows 2 entries, not 4; (b) items with distinct names -> unchanged behavior; (c) the prompt does not contain the candidate list twice.
<!-- AC:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
FindSong disambiguation deduplicates by name + fixes double-speak. Gates: bug review (zero bugs), /simplify 4-agent (all actionable findings applied). Suite 2762 green.
