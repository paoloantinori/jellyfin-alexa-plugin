...
public abstract class BaseHandler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        SkillResponse response = Launch.BuildAudioPlayerResponse(PlayBehavior.ReplaceAll, Launch.GetStreamUrl(itemId, user), itemId, sortedItems[startIndex], user, context, announceLocale: locale);
        ...
        string? announcement = null)
    {
        ...
        SkillResponse response = Launch.BuildAudioPlayerResponse(
            PlayBehavior.ReplaceAll, Launch.GetStreamUrl(itemId, user), itemId, song, user, context, announceLocale: locale);
        ApplyAnnouncement(response, announcement);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        SkillResponse albumResponse = Launch.BuildAudioPlayerResponse(PlayBehavior.ReplaceAll, Launch.GetStreamUrl(item_id, user), item_id, albumItems[startIndex], user, context, announceLocale: locale);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (playlists.TotalRecordCount > 1)
        {
            ...
            else
            {
                ...
                {
                    var matches = playlists.Items.Take(3).Select(p => (p.Id, p.Name, (string?)Launch.GetImageUrl(p.Id.ToString("N"), user))).ToList();
