...
public class PlayChannelIntentHandler : BaseHandler
{
    ...
    public override async Task<SkillResponse> HandleAsync(Request request, Context context, Entities.User user, SessionInfo session, CancellationToken cancellationToken)
    {
        ...
        if (channels.Count == 0)
        {
            var fuzzy = await Search.SearchItemsFuzzyAsync(channelQuery, jellyfinUser, user, _libraryManager, new[] { BaseItemKind.LiveTvChannel }, cancellationToken, "PlayChannelFuzzyFallback", locale: locale).ConfigureAwait(false);
            if (fuzzy != null)
            {
