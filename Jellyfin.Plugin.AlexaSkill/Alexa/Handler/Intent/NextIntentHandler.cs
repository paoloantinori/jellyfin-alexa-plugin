    ...
    public override Task<SkillResponse> HandleAsync(Request request, Context context, Entities.User user, SessionInfo session, CancellationToken cancellationToken)
    {
        ...
        for (int i = 0; i < session.NowPlayingQueue.Count - 1; i++)
        {
            if (session.NowPlayingQueue[i].Id == session.FullNowPlayingItem?.Id)
            {
                ...
                AudioLaunchSource source = ResolveAudioLaunchSource(nextItem, item_id, user, 0, context?.System?.Device?.DeviceID);
                return Task.FromResult<SkillResponse>(BuildAudioPlayerResponse(PlayBehavior.ReplaceAll, source.Url, item_id, nextItem, user, context));
            }
        }
