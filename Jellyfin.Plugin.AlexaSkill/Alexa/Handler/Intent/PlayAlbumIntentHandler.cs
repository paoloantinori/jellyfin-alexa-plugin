...
public class PlayAlbumIntentHandler : BaseHandler
{
    ...
    /// Reuses the same artist-songs playback logic as PlayArtistSongsIntentHandler.
    /// </summary>
    private async Task<SkillResponse> PlayArtistSongsFromAlbumFallback(
        Guid artistId,
        string artistName,
        ...
        string? announcement = null)
    {
        ...
        IReadOnlyList<BaseItem> artistItems = await RetryAsync(
            ...
            "GetArtistSongsFromAlbumFallback",
            cancellationToken).ConfigureAwait(false);

        Logger.LogDebug("PlayAlbum fallback: fetched {Count} songs for artist='{Artist}'", artistItems.Count, artistName);
        ...
        Logger.LogDebug(
            "PlayAlbum fallback: returning AudioPlayer, itemId={ItemId}, startIndex={StartIndex}, queueSize={QueueSize}",
        ...
        // If an announcement was provided, add it as speech output.
